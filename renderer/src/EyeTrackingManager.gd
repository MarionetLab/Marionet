...
func update_eye_tracking():
	...
	# 只使用GDCubismEffectTargetPoint，避免参数冲突
	# 注释掉强制参数设置，让GDCubismEffectTargetPoint自己处理所有眼动追踪
	# if gd_cubism_target_point:
	#	var model = gd_cubism_target_point.get_parent()
	#	if model:
	#		# 每30帧（约0.5秒）设置一次参数，避免干扰待机动画
...
func find_eye_tracking_node():
	...
	if gd_cubism_target_point:
		...
		# 检查GDCubismEffectTargetPoint支持的所有属性
		print("GDCubismEffectTargetPoint 属性检查:")
		print("  - eyes_range: ", gd_cubism_target_point.get("eyes_range"))
		print("  - head_range: ", gd_cubism_target_point.get("head_range"))
		print("  - body_range: ", gd_cubism_target_point.get("body_range"))
		print("  - smooth_factor: ", gd_cubism_target_point.get("smooth_factor"))
		print("  - max_distance: ", gd_cubism_target_point.get("max_distance"))
...
func force_set_eye_tracking_parameters():
	...
	# 只设置眼动追踪相关的参数，不干扰待机动画（呼吸、眨眼等）
	var params = model.get_parameters()
	for param in params:
		var param_name = param.get_id()
		# 只设置角度参数，不设置眼球参数（让GDCubismEffectTargetPoint处理眼球）
		if param_name == "ParamAngleX":
			# 头部左右转动：-25到25度
...
# 呼吸参数监控函数（仅用于调试，不修改任何参数）
func monitor_breath_parameters():
	if not main_scene:
		return
	...
	for param in params:
		var param_name = param.get_id()
		if "breath" in param_name.to_lower() or "breathing" in param_name.to_lower():
			breath_params.append("%s: %.3f" % [param_name, param.get_value()])
	
	# 输出呼吸参数状态（调试用，限制频率）
	if breath_params.size() > 0 and Engine.get_process_frames() % 300 == 0:  # 每5秒输出一次
		print("呼吸参数状态: ", breath_params)
